...
handlers._checks.put = function (data, callback) {
  ...
  const checkId =
    data.queryStringObject.has("id") && typeof data.queryStringObject.get("id") == "string" &&
  ...
  if (checkId) {
    ...
    if (newProtocol || newUrl || newMethod || newSuccessCodes || newTimeoutSeconds) {
                ...
                if (!err) {
                  ...
                  if (userData.checks.indexOf(checkData.id) > -1) {
                    newCheckData = {
                      ...
                      "timeoutSeconds": newTimeoutSeconds || checkData.timeoutSeconds
                    }
