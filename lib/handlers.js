        ...
        _data.read("users", phone, function (err, userData) {
          if (!err) {
            ...
            _data.delete("users", phone, function (err) {
              if (!err) {
                if (typeof userChecks == 'object' &&
                  userChecks instanceof Array &&
                  userChecks.length > 0) {
                  let deletionFailures = [];
                  let failureHit = false;
                    while (userChecks.length) {
                      let nextToDelete = userChecks.shift();
                      _data.delete("checks", nextToDelete, function (err) {
                        if (err) {
                          deletionFailures.push(nextToDelete);
                        }
                        if (!userChecks.length) {
                          if (deletionFailures.length) {
                            callback(500, { "Message": "Checks not found", "ids": deletionFailures })  
                          } else {
                            callback(200);
                          }
                        }
  ...
  if (id) {
    ...
    _data.read("checks", id, function (err, checkData) {
      if (!err) {
        ...
        callback(403, { Message: "Check not found" });
      }
    });
  } else {
    callback(400, { Message: "Missing field" });
                ...
                if (isValid) {
                  if (
                    ...
                    _data.delete("checks", id, function (err) {
                      if (!err) {
                        const purged = userData.checks.splice(userData.checks.indexOf(id),1
                        );
                        _data.update(
                          "users",
                          userData.phone,
